...
import { getSalesOrder } from '../../Service/api';
import SoModal from "../OurModal/SoModal"
...
const SoTable = (props) => {
  ...
  const getSalesQuotationDatas = async () => {
    let response = await getSalesOrder();
    if (response) {
  ...
  if (searchResults.length > 0) {
    ...
    resultData = searchResults.map((salesQuanty) => (
      <tr key={salesQuanty.DocNum} className={salesQuanty.selected ? "selected" : ""}>
        ...
        <td>{salesQuanty.U_EndCusCode}</td>
        <td>{salesQuanty.DocTotal}</td>
        <td>{salesQuanty.DocStatus}</td>
      </tr>
    ))
  } else {
    ...
    <div className="container">
      <div className="row">
        ...
        <div className="col-md-4 col-sm-4">
          <SoModal onClick={() => getSelectedRows(SelectedList)} clickedData={SelectedList} main={SelectedList.length}>
