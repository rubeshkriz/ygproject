...
import { customerOutsanding } from '../../Service/api';
import CoModal from "../OurModal/CoModal"
...
const CoTable = (props) => {
  ...
  const getSalesQuotationDatas = async () => {
    let response = await customerOutsanding();
    if (response) {
  ...
  if (searchResults.length > 0) {
    ...
    resultData = searchResults.map((salesQuanty) => (
      <tr key={salesQuanty.DocNum} className={salesQuanty.selected ? "selected" : ""}>
        ...
        <td>{salesQuanty.BalaneAmount}</td> 
      </tr>
    ))
  } else {
    ...
    <div className="container">
      <div className="row">
        ...
        <div className="col-md-4 col-sm-4">
          <CoModal onClick={() => getSelectedRows(SelectedList)} clickedData={SelectedList} main={SelectedList.length}>
